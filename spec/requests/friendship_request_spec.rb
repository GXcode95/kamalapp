...
RSpec.describe FriendshipsController, type: :request do
  ...
  let!(:accepted_friendship_user1_user2) { create(:accepted_friendship, user: user1, friend: user2) }

  let!(:pending_friendship_user1_user2_sent) { create(:pending_friendship, user: user1, friend: user2) }
  ...
  let!(:pending_friendship_user3_user1_received) { pending_friendship_user3_user1_sent.reciprocal_friendship }
  ...
  context 'when user is signed in' do
    ...
    describe 'GET index' do
      it 'assigns @accepted_friendships' do
        ...
        expect(assigns(:accepted_friendships)).to eq([accepted_friendship_user1_user2, pending_friendship_user1_user2_sent])
      end

      it 'assigns @pending_friendships' do
        get friendships_path
        expect(assigns(:pending_friendships)).to eq([pending_friendship_user3_user1_received])
      end
    ...
    describe 'POST create' do
      ...
      context 'when friendship already exists' do
        ...
        it 'accept the friendship' do
          post friendships_path(format: :turbo_stream), params: existing_friendship_params
          expect(pending_friendship_user3_user1_received.reload.status).to eq('accepted')
        end
    ...
    describe 'PATCH update' do
      it 'accept the friendship' do
        patch friendship_path(pending_friendship_user2_user1_received, format: :turbo_stream)
        expect(pending_friendship_user2_user1_received.reload.status).to eq('accepted')
      end

      it 'returns a turbo_stream tag with update action for friendship' do
        patch friendship_path(pending_friendship_user2_user1_received, format: :turbo_stream)
        expect(response.body).to(
          include("<turbo-stream action=\"update\" target=\"friendship_#{pending_friendship_user2_user1_received.id}\"")
        )
      end
      ...
      it 'returns a turbo_stream tag with update action for user' do
        patch friendship_path(pending_friendship_user2_user1_received, format: :turbo_stream)
        expect(response.body).to(
          include("<turbo-stream action=\"update\" target=\"user_#{user1.id}\"")
        )
      end
    ...
    describe 'DELETE destroy' do
      it 'delete the friendship' do
        delete friendship_path(pending_friendship_user3_user1_received, format: :turbo_stream)
        expect { pending_friendship_user3_user1_received.reload }.to raise_error(ActiveRecord::RecordNotFound)
      end

      it 'delete the reciprocal friendship friendship' do
        delete friendship_path(pending_friendship_user3_user1_received, format: :turbo_stream)
        expect { pending_friendship_user3_user1_received.reload.reciprocal_friendship }.to raise_error(ActiveRecord::RecordNotFound)
      end

      it 'returns a turbo_stream tag with remove action' do
        delete friendship_path(pending_friendship_user2_user1_received, format: :turbo_stream)
        expect(response.body).to(
          include("<turbo-stream action=\"remove\" target=\"friendship_#{pending_friendship_user2_user1_received.id}\"")
        )
      end
